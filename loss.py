...
class StyleReconstructionRegularizer(Regularizer):
    ...
    def __call__(self, x):
        ...
        loss = self.weight * K.mean(K.sum(K.square(gram_matrix(output) - gram_matrix(self.style_feature_target))))  /  K.cast((channels * size * size), dtype='float32')
...
class TVRegularizer(Regularizer):
    ...
    def __call__(self, x):
        ...
        shape = K.shape(x_out)
        img_width, img_height = (shape[1],shape[2])
